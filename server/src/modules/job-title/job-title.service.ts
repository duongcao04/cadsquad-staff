...
export class JobTitleService {
  ...
  async create(data: CreateJobTitleDto): Promise<JobTitle> {
    ...
    return plainToInstance(JobTitleResponseDto, jobTitle, { excludeExtraneousValues: true }) as unknown as JobTitle
  }

  async findAll(): Promise<JobTitle[]> {
    const jobTitles = await this.prismaService.jobTitle.findMany()
    return jobTitles.map(jt => plainToInstance(JobTitleResponseDto, jt, { excludeExtraneousValues: true })) as unknown as JobTitle[]
  }

  async findById(id: string): Promise<JobTitle> {
    const jobTitle = await this.prismaService.jobTitle.findUnique({ where: { id } })
  ...
  async update(id: string, data: UpdateJobTitleDto): Promise<JobTitle> {
    try {
      const updated = await this.prismaService.jobTitle.update({ where: { id }, data })
      return plainToInstance(JobTitleResponseDto, updated, { excludeExtraneousValues: true }) as unknown as JobTitle
    } catch (error) {
