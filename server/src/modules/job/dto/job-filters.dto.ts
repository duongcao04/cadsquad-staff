...
export class JobFiltersDto {
   ...
   */
  @ApiProperty({
    description: 'Filter by job status code. Can be a comma-separated list.',
    required: false,
    type: String,
  })
  @IsOptional()
   ...
   */
  @ApiProperty({
    description: 'Filter by assignee username. Can be a comma-separated list.',
    required: false,
    type: String,
  })
  @IsOptional()
   ...
   */
  @ApiProperty({
    description: 'Filter by payment channel code. Can be a comma-separated list.',
    required: false,
    type: String,
  })
  @IsOptional()
...
export class JobFiltersBuilder {
  ...
  static build(filters: JobFiltersDto): Prisma.JobWhereInput {
    ...
    if (filters.paymentChannel?.length) {
      where.paymentChannel = {
        displayName: { in: filters.paymentChannel, mode: 'insensitive' },
      }
    }

    // 6. Date Ranges
    if (filters.createdAtFrom || filters.createdAtTo) {
      where.createdAt = this.mapDateRange(filters.createdAtFrom, filters.createdAtTo)
    }
    ...
    if (filters.completedAtFrom || filters.completedAtTo) {
      where.completedAt = this.mapDateRange(filters.completedAtFrom, filters.completedAtTo)
    }
    if (filters.finishedAtFrom || filters.finishedAtTo) {
      where.finishedAt = this.mapDateRange(filters.finishedAtFrom, filters.finishedAtTo)
    }
   ...
   */
  private static mapDateRange(from?: string, to?: string): Prisma.DateTimeFilter | undefined {
   ...
   */
  private static mapNumberRange(min?: string, max?: string): Prisma.IntFilter | undefined {
