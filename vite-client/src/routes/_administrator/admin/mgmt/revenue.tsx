...
const RECENT_TRANSACTIONS = [
  { id: 1, job: "FV-2024", client: "TechCorp", date: "2024-07-15", amount: 4500, status: "Paid" },
  { id: 2, job: "FV-2025", client: "Amoovo", date: "2024-07-14", amount: 2800, status: "Paid" },
  { id: 3, job: "FV-2026", client: "StartUp Inc", date: "2024-07-12", amount: 1200, status: "Pending" },
  { id: 4, job: "FV-2027", client: "Retail Chain", date: "2024-07-10", amount: 8500, status: "Paid" },
  { id: 5, job: "FV-2028", client: "Logistics Co", date: "2024-07-08", amount: 3100, status: "Overdue" },
];
...
const KpiCard = ({ title, value, subValue, trend, icon: Icon, color }: any) => (
  <Card className="w-full" shadow="sm">
    <CardBody className="flex flex-row items-center justify-between p-6">
      <div>
        <p className="text-default-500 text-sm font-medium uppercase tracking-wide">{title}</p>
        <h3 className="text-2xl font-bold text-slate-800 mt-1">{value}</h3>
        <div className={`flex items-center gap-1 mt-2 text-xs font-semibold ${trend === `up` ? `text-success` : `text-danger`}`}>
    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Financial analytics overview for income, staff costs, and expenses.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-4" shadow="sm">
          <h3 className="text-lg font-bold text-slate-800 mb-2 px-2">Revenue by Service</h3>
      ...
      <Card className="w-full" shadow="sm">
        <CardBody className="p-0 overflow-hidden">
            ...
            <Table aria-label="Recent transactions" shadow="none" removeWrapper>
            <TableHeader>
            ...
            <TableBody>
                ...
                <TableRow key={row.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    <TableCell className="font-medium text-slate-700">{row.job}</TableCell>
                    <TableCell className="text-slate-500">{row.client}</TableCell>
                    <TableCell className="text-slate-500 flex items-center gap-2">
                        <CalendarIcon size={14} className="text-slate-400" /> {row.date}
                    </TableCell>
                    <TableCell>
                        <Chip 
                            size="sm" 
