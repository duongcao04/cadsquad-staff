...
function SchedulePage() {
    ...
    const handleEventClick = (job: JobEvent) => {
        setSelectedJob(job)
        onOpen()
    }

    return (
        <div className="p-8 h-screen flex flex-col max-w-[1600px] mx-auto bg-slate-50">
            {/* --- Header Toolbar --- */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 shrink-0">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">
                        Manage project timelines and team availability.
                    </p>
                </div>
            ...
            <Card className="flex-1 flex flex-col shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50 shrink-0">
                    {weekDays.map((day) => (
                        <div
                            key={day}
                            className="py-3 text-center text-sm font-semibold text-slate-500 uppercase tracking-wider"
                        >
                            {day}
                        </div>
                    ))}
                    ...
                    {calendarDays.map((day, idx) => {
                        ...
                        return (
                            <div
                                key={day.toISOString()}
                                className={`
                  relative bg-white flex flex-col p-2 min-h-[120px] transition-colors
                                ...
                                <div className="flex-1 flex flex-col gap-1 overflow-y-auto max-h-[120px] custom-scrollbar">
                                    {daysJobs.map((job) => (
                                        <div
                                            ...
                                            className="group flex items-center gap-2 p-1.5 rounded-lg text-xs font-medium cursor-pointer border border-transparent hover:border-slate-200 hover:shadow-sm transition-all bg-opacity-10"
                                            style={{
                            ...
                            <ModalBody>
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <span className="text-sm text-slate-500">
                                        for{' '}
                                        <strong>{selectedJob?.client}</strong>
                                    </span>
                                </div>
