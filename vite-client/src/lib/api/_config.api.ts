...
export const configApi = {
    create: (data: TCreateUserConfigInput) => {
        return axiosClient.post<ApiResponse<IConfigResponse>>(
            '/v1/configs',
            data
        )
    },
    ...
    findById: (id: string) => {
        return axiosClient.get<ApiResponse<IConfigResponse>>(
            `/v1/configs/${id}`
        )
    },
    findByCode: (code: string) => {
        return axiosClient.get<ApiResponse<IConfigResponse>>(
            `/v1/configs/code/${code}`
        )
    },
    updateByCode: (code: string, data: TUpdateUserConfigInput) => {
        return axiosClient.patch<ApiResponse<{ code: string }>>(
            `/v1/configs/code/${code}`,
            data
        )
    },
