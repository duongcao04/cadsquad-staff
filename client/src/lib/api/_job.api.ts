...
import { TBulkChangeStatusInput, TChangeStatusInput, TCreateJobInput, TJobQueryWithFiltersInput, TRescheduleJob, TUpdateJobInput, TUpdateJobMembersInput } from '@/lib/validationSchemas'
...
export const jobApi = {
	...
	getAssignees: (id: string) => {
		return axiosClient.get<ApiResponse<{ assignees: IUserResponse[], totalAssignees: number }>>(`/v1/jobs/${id}/assignees`)
	},
	getJobActivityLog: (id: string) => {
		return axiosClient.get(`/v1/jobs/${id}/activity-log`)
	},
	changeStatus: (id: string, data: TChangeStatusInput) => {
		return axiosClient.patch<ApiResponse<{ id: string, no: string }>>(`/v1/jobs/${id}/change-status`, data)
	},
	reschedule: (id: string, data: TRescheduleJob) => {
		return axiosClient.patch<ApiResponse<{ id: string, no: string }>>(`/v1/jobs/${id}/reschedule`, data)
	},
	...
	update: (id: string, data: TUpdateJobInput) => {
		return axiosClient.patch<ApiResponse<{ id: string, no: string }>>(`/v1/jobs/${id}`, data)
	},
	remove: (jobId: string) => {
