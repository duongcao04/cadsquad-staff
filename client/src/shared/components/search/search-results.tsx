...
export const SearchResults = ({ query, category, onSelect }: SearchResultsProps) => {
  ...
  const getIcon = (type: string) => {
    switch(type) {
        case 'Jobs': return <BriefcaseIcon size={16} />;
        case 'Clients': return <UserCircleIcon size={16} />;
        case 'System': return <SettingsIcon size={16} />;
        case 'Communities': return <UsersIcon size={16} />;
        case 'Staff Members': return <BadgeCheck size={16} />;
    ...
    >
      {data.map((item: any) => (
        <ListboxItem
          ...
          endContent={<ChevronRightIcon size={14} className="text-default-300 opacity-0 group-hover:opacity-100 transition-opacity" />}
        >
