...
import { TBrowserSubscribe } from '../../../../shared/types'
...
export async function POST(request: NextRequest) {
    ...
    const browserSubscribes = await axiosClient
        .get<ApiResponse<TBrowserSubscribe[]>>('/browser-subscribes')
        .then((res) => {
    ...
    if (subscriptionsToDelete.length > 0) {
        console.log(
            `Deleting ${subscriptionsToDelete.length} expired subscriptions.`
        )
        // Fire off delete requests in parallel
        await Promise.all(
            subscriptionsToDelete.map((endpoint) =>
                axiosClient.delete('/browser-subscribes/by-endpoint', {
                    data: { endpoint },
                })
            )
        )
